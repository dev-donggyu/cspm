insert into cspm.policy (id ,policyType, policyTitle, policySeverity, policyDescription, policyResponse, policyCompliance,
                         policyPattern)
VALUES ('1',
        '접근 관리', '보안그룹 Inbound Any open(0.0.0.0) 점검', 'High',
        ...
        'Select p.group_id AS resourceId, p.scan_time FROM resource_security_group p WHERE p.ip_permissions LIKE ''%CidrIp=0.0.0.0/0%'';'),
       ('2',
        '접근 관리', '보안그룹 Outbound Any open(0.0.0.0) 점검', 'Medium',
        ...
        'Select p.group_id AS resourceId, p.scan_time FROM resource_security_group p WHERE p.ip_permissions_egress LIKE ''%CidrIp=0.0.0.0/0%'';'),
       ('3',
        '접근 관리', '인스턴스 Key-pair 미사용', 'High',
        ...
        'PCI DSS 6.5.10, ISO/IEC 27001:2013 A.9.4, NIST SP 800-53 SC-13, CIS AWS Foundations Benchmark 4.1',
        'Select p.instance_id AS resourceId, p.scan_time FROM resource_instance p WHERE p.key_name='''' OR p.key_name IS NULL'),
       ('4',
        '설정 관리', '미사용 ENI 점검', 'Low',
        ...
        'Select p.eni_id AS resourceId, p.scan_time FROM resource_eni p WHERE status=''available'' AND (attachment='''' OR attachment IS NULL) AND (instance_id='''' OR instance_id IS NULL)'),
       ('5',
        '설정 관리', '암호화되지 않은 EBS 점검', 'Low',
        ...
        'Select p.ebs_id AS resourceId, p.scan_time FROM resource_ebs p WHERE p.encrypted=''0'';'),
       ('6','암호화', '미사용 EBS 볼륨 탐지', 'Medium',
        ...
        'ISO/IEC 27001:2013 A.12.6.1, CIS AWS Foundations Benchmark 2.9', 'Select p.ebs_id AS resourceId, p.scan_time FROM resource_ebs p WHERE p.state = ''available'' AND p.attachments = ''[]'';'),
       ('7','설정 관리', '서울 리전이 아닌 인스턴스 탐지', 'Medium',
        ...
        'Select p.instance_id AS resourceId, p.scan_time FROM resource_instance p WHERE p.placement LIKE ''%AvailabilityZone=ap-northeast-2%'''),
       ('8','설정 관리', 'S3 Public 설정 여부', 'High',
        ...
        'Select p.s3_id AS resourceId, p.scan_time FROM resource_s3 p WHERE p.public_access_block_configuration LIKE ''%BlockPublicAcls=false%''
  AND p.public_access_block_configuration LIKE ''%IgnorePublicAcls=false%''
  AND p.public_access_block_configuration LIKE ''%BlockPublicPolicy=false%''
  AND p.public_access_block_configuration LIKE ''%RestrictPublicBuckets=false%'';'),
       ('9','설정 관리', 'ENI Public IP 탐지', 'Medium',
        ...
        'Select p.eni_id AS resourceId, p.scan_time FROM resource_eni p
WHERE NOT (
    p.private_ip_address LIKE ''10.%'' OR
    (p.private_ip_address LIKE ''172.16.%'' OR
     p.private_ip_address LIKE ''172.17.%'' OR
     p.private_ip_address LIKE ''172.18.%'' OR
     p.private_ip_address LIKE ''172.19.%'' OR
     p.private_ip_address LIKE ''172.20.%'' OR
        p.private_ip_address LIKE ''172.21.%'' OR
        p.private_ip_address LIKE ''172.22.%'' OR
        p.private_ip_address LIKE ''172.23.%'' OR
        p.private_ip_address LIKE ''172.24.%'' OR
        p.private_ip_address LIKE ''172.25.%'' OR
        p.private_ip_address LIKE ''172.26.%'' OR
        p.private_ip_address LIKE ''172.27.%'' OR
        p.private_ip_address LIKE ''172.28.%'' OR
        p.private_ip_address LIKE ''172.29.%'' OR
        p.private_ip_address LIKE ''172.30.%'' OR
        p.private_ip_address LIKE ''172.31.%'') OR
       p.private_ip_address LIKE ''192.168.%''
);'),
       ('10','설정 관리', '미사용 IGW 탐지', 'Medium',
        ...
        'Select p.igw_id AS resourceId, p.scan_time FROM resource_igw p where p.attachments=''[]'';'),
       ('11','설정 관리', 'Instance Public IP 탐지', 'Los',
        ...
        'Select  p.instance_id AS resourceId, p.scan_time FROM resource_instance p
 WHERE p.public_address!='''';'),
       ('12','계정 관리', '라우팅 테이블 0.0.0.0 탐지', 'Low',
        ...
        'Select  p.route_id AS resourceId, p.scan_time FROM resource_route p
 WHERE p.routes LIKE ''%DestinationCidrBlock=0.0.0.0/0%'';');
--        ('계정 관리', 'MFA 사용 여부', 'Medium', '', '', '', ''),
--        ('계정 관리', '30일 이상 장기 미사용자 점검', 'Medium', '', '', '', ''),
--        ('계정 관리', 'IAM 암호 정책(암호 강도 설정)', 'Medium', '', '', '', ''),
--        ('계정 관리', 'IAM 암호 정책(암호 만료 활성화)', 'Medium', '', '', '', ''),
--        ('계정 관리', 'IAM 암호 정책(암호 재사용 제한)', 'Medium', '', '', '', ''),
--        ('계정 관리', 'IAM 암호 정책(암호 최소길이)', 'Medium', '', '', '', ''),
--        ('계정 관리', 'Access key 90일 이상 미사용', 'Medium', '', '', '', ''),
--        ('접근 관리', 'NACL In/Out Bound Any open', 'Medium', '', '', '', ''),
--        ('암호화', 'EBS 스냅샷 암호화 여부', 'Medium', '', '', '', ''),
--        ('로그 관리', 'VPC Flow Log 설정', 'Medium', '', '', '', ''),
--        ('설정 관리', 'S3 버킷 MFA Delete 활성화 여부', 'Medium', '', '', '', ''),
--        ('로그 관리', 'Cloudtrail 로그 Cloudwatch 통합 여부', 'Medium', '', '', '', ''),
--        ('로그 관리', '그룹 이름에 admin이 있는 경우', 'Medium', '', '', '', ''),
--        ('암호화', '취약한 Security Policy를 사용하는 ELB 점검', 'Medium', '', '', '', '');
